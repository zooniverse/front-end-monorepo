/** Same helper function as lib-user's BarChart */

export default function getCompleteData(data, dateInterval) {
  const completeData = []
  if (data.length > 0) {
    ...
    // Looking at a data object of { count, period } get the year month day of each stats bucket
    // and compare those to the `period` of dateInterval. If there's a match,
    // include it in the data passed to BarChart. Otherwise we add count = 0.
    while (currentDate <= endDate) {
      ...
      // Increment. Index is used by BarChart's axis labels.
      index += 1
    }
  }
  return completeData
