...
const PlaceholderBox = styled(Box)`
  background-image: url("data:image/svg+xml;base64,${typeof btoa !== 'undefined' ? btoa(svgAudioIcon) : ''}");
  background-size: 50%;
...
function SingleAudioViewer({
  ...
  const handleTimeUpdate = () => {
    if (!isSeeking && audioRef.current && duration > 0) {
  ...
  return (
    <Box width='100%'>
      ...
      <PlaceholderBox
        background={{ light: 'dark-5', dark: 'dark-1' }}
        width='100%'
      />
