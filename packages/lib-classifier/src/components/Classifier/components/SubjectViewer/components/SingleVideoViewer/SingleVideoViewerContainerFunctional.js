...
function SingleVideoViewerContainer({
  ...
  const onReactPlayerReady = () => {
    try {
      ...
      const target = { svgClientHeight, svgClientWidth, reactPlayerVideoWidth, reactPlayerVideoHeight }
      onReady({ target })
    } catch (error) {
      onError(error)
    }
  ...
  const videoSrc = Object.values(subject?.locations[0])[0]
  ...
  return (
    <>
      {videoSrc
        ...
        : (
          <Box>Unable to load the subject</Box>
          )}
      <VideoController
        isPlaying={isPlaying}
        played={timeStamp}
