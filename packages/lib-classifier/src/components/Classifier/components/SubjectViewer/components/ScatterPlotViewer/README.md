...
- `xAxisLabelOffset` _(number)_ Default: `undefined`. The offset of the x-axis label. We rely on vx to position the label for us, but if it needs adjustment, then an offset can be set. This can be set by the subject location JSON's `chartOptions` property (See the section on [JSON file](#JSON_file)).
...
- `yAxisLabelOffset` _(number)_ Default: `undefined`. The offset of the y-axis label. We rely on vx to position the label for us, but if it needs adjustment, then an offset can be set. This can be set by the subject location JSON's `chartOptions` property (See the section on [JSON file](#JSON_file)).
