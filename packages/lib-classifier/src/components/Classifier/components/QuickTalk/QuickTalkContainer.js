...
class QuickTalkContainer extends React.Component {
  ...
  async checkUser () {
    ...
    this.setState({
      ...
      bearerToken: (authorization && user) ? authorization : undefined
    })
  }
  ...
  /*
  Authentication data in talkClient.headers is lost when user is logged in
  via app-client instead of lib-classifier. Reasons are unknown so far, but
  this fixes it.
   */
  fixTalkClientAuth () {
    if (!talkClient?.headers) return
    if (this.state.bearerToken) {
      talkClient.headers.Authorization = this.state.bearerToken
    } else {
