...
describe('StepNavigation', function () {
  it('should render no buttons and no inputs if there are no steps', function () {
    ...
    expect(screen.queryByRole('button', { name: 'Go to previous step' })).to.equal(null)
    expect(screen.queryByRole('button', { name: 'Go to next step' })).to.equal(null)
  ...
  it('should render a previous button', function () {
    ...
    expect(screen.getByRole('button', { name: 'Go to previous step' })).to.exist
  })

  it('should render a next button', function () {
    render(<StepNavigation steps={steps} />)
    expect(screen.getByRole('button', { name: 'Go to next step' })).to.exist
  })
  ...
  it('should disable the previous step button when stepIndex is 0', function () {
    render(<StepNavigation steps={steps} />)
    expect(screen.getByRole('button', { name: 'Go to previous step' }).disabled).to.equal(true)
  })
  ...
  it('should disable the next step button when stepIndex is the last step', function () {
    ...
    expect(screen.getByRole('button', { name: 'Go to next step' }).disabled).to.equal(true)
  })
