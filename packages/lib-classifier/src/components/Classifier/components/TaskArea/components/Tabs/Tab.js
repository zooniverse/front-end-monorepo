...
function getTabColor(color) {
  return theme('mode', {
    dark: zooTheme.global.colors[color.dark],
    light: zooTheme.global.colors[color.light]
  })
}
...
function getHoverStyles(which, colorTheme) {
  ...
  console.log(styles[which][colorTheme])
...
const StyledTab = styled(Tab)`
  background: ${props => props.active
    ? getTabColor({ light: 'white', dark: 'dark-3' })
    : getTabColor({ light: 'light-1', dark: 'dark-2' })
};
  color: ${props => props.active
    ? getTabColor({ light: 'black', dark: 'light-1' })
    : getTabColor({ light: 'dark-5', dark: 'light-5' })
};
  flex: 1;
  height: 100%;
  max-height: 55px;
  ...
  &:last-of-type {
    border-left: 1px solid ${getTabColor({ light: 'light-3', dark: 'dark-1' })};
  }
